...
public class ScsCredhubEnvironmentRepository implements EnvironmentRepository {
   ...
   public Environment findOne(String application, String profilesList, String label) {
    log.info("loading environment");
      ...
      for(int var8 = 0; var8 < var7; ++var8) {
         ...
         Map properties = this.findProperties(application, profile, label);
   ...
   private Map findProperties(String application, String profile, String label) {
      String path = "/" + this.credHubPath + "/" + application + "/" + profile + "/" + label;
      return (Map)this.credHubOperations.credentials().findByPath(path).stream().map((credentialSummary) -> {
