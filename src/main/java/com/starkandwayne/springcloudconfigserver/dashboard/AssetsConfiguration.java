...
public class AssetsConfiguration extends WebSecurityConfigurerAdapter {
   ...
   protected void configure(HttpSecurity http) throws Exception {
      ((ExpressionUrlAuthorizationConfigurer.AuthorizedUrl)((HttpSecurity)http.antMatcher(this.ASSETS_URL_PREFIX).headers().cacheControl().disable().and()).authorizeRequests().anyRequest()).permitAll();
   }
}