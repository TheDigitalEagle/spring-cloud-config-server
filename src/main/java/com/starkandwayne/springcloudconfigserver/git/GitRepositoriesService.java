...
public class GitRepositoriesService {
   ...
   @Autowired
   public GitRepositoriesService(Environment environment, List environmentRepositories) {
      this.environmentRepositories = environmentRepositories;
      BindResult compositeBinder = Binder.get(environment).bind("spring.cloud.config.server.composite", Bindable.listOf(Repository.class));
      BindResult gitBinder = Binder.get(environment).bind("spring.cloud.config.server.git", Repository.class);
      if (compositeBinder.isBound()) {
         this.repositories = (List)compositeBinder.get();
   ...
   public List getDefaultBranchCommitIds() {
      ...
      return (List)this.repositories.stream().filter((repo) -> {
         return "git".equals(((Bindable) repo).getType());
